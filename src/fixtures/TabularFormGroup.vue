...
type GroupItem = { parent: string; children: ChildrenItem[] };
type ChildrenItem = { firstField: string; secondField: string };
...
const schema = useSchema(
  [
    [
      ...
      (row: GroupItem, idx: number) => [
        [computed(() => row.parent), string().required()],
        [
          computed(() => row.children),
          (subRow: ChildrenItem, subIdx: number) => [
